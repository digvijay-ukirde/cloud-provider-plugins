...
class DBManager:
    ...
    def _attempt_json_recovery(self, filepath: str) -> Optional[Dict[str, Any]]:
        ...
        try:
            ...
            # start_idx and ignores any trailing garbage
    ...
    def create_request(self, request_id: str, template_id: str,
                    ...
                    fleet_type: Optional[str] = None) -> bool:
    ...
    def remove_request(self, request_id: str) -> bool:
        """Remove an empty request entry from the database."""
        with self._lock:
            with self._file_lock_context() as lock_acquired:
                if not lock_acquired:
                    logger.error("remove_request: could not acquire file lock")
                    return False
                ...
                try:
                    ...
                    for i, request in enumerate(data.get('requests', [])):
                        if request['requestId'] == request_id:
                            if request.get('machines'):
                                logger.warning(
                                    f"remove_request: refusing to remove {request_id} "
                                    f"— it still has {len(request['machines'])} machine(s)"
                                )
                                return False
                            data['requests'].pop(i)
                            self._atomic_write(self.db_file, data, backup=False)
                            logger.info(f"Removed empty request {request_id} from database")
                            return True

                    logger.debug(f"remove_request: {request_id} not found — nothing to remove")
                    return False
    ...
    def cleanup_old_data(self, max_request_age_minutes: int = 60) -> Dict[str, Any]:
        ...
        with self._lock:
            with self._file_lock_context() as lock_acquired:
                if not lock_acquired:
                    logger.error("cleanup_old_data: could not acquire file lock")
                    return stats
                ...
                try:
                    ...
                    for request in data.get('requests', []):
                        ...
                        if current_count == 0 and is_old_request:
                            ...
                            if request['requestId'].startswith(('fleet-', 'sfr-')):
                                stats['removed_fleet_requests'].append({
                    ...
                    if stats['empty_requests_removed'] > 0 or stats['terminated_machines_removed'] > 0:
                        ...
                        self._atomic_write(self.db_file, data, backup=False)
                        logger.info(
                            f"Cleanup removed {stats['empty_requests_removed']} empty requests "
                            f"and {stats['terminated_machines_removed']} terminated machines"
                        )
