...
from aws_rc_instance import RcInOut
from aws_rc_config import AWSTemplate, AWSConfig, get_aws_configs, get_aws_template
from aws_logger import set_rc_logger
from aws_utils import AWSVPCManager
...
def main():
    ...
    template = get_aws_template(request.templateId)
    ...
    manager = AWSVPCManager(config, template)
    instanceList, error = manager.request_instances(request.machineCount, request.tagValue)
    logging.info("%s instances successfully created (pid=%s)" % (len(instanceList), os.getpid()))
    
    if (len(instanceList) <= 0):
        logging.error("Fail to create any EC2 instance")
    ...
    if work_dir is None:
        ...
        logging.info("The LSF_SHAREDIR env. variable is not set. Using " + work_dir)
...
if __name__ == "__main__":
    ...
    try:
        ...
        if len(createdInstanceList) > 0:
            ...
            AWSVPCManager.delete_resources_from_vms(createdInstanceList)
